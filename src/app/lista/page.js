...
export default function ListaPage() {
  ...
  const cargarNegocios = async () => {
    ...
    const { data, error } = await supabase
      .from('negocios')
      .select('*, precios(*)')
    if (!error && data) setNegocios(data)
    setLoading(false)
  }
  ...
  lista = [...lista].sort((a, b) => {
    ...
    if (sort === 'economico') return (a.precios?.[0]?.precio || 0) - (b.precios?.[0]?.precio || 0)
    if (sort === 'caro') return (b.precios?.[0]?.precio || 0) - (a.precios?.[0]?.precio || 0)
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 py-3 flex flex-col gap-3">
        {loading ? (
          <div className="flex flex-col items-center justify-center flex-1 py-16 gap-3">
            ...
            <p className="text-[12px] text-muted font-light">Cargando negocios...</p>
          </div>
        ) : lista.map(n => (
            ...
            <div className="bg-card rounded-2xl border border-border overflow-hidden">
              <div className="px-4 pt-3 pb-2 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted font-light">{n.tipo === 'restaurante' ? 'Restaurante' : 'Tienda'} · {n.municipio}</p>
                </div>
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <span className={`text-[12px] font-bold font-serif px-2 py-0.5 rounded-lg ${scoreColor(n.puntuacion)}`}>
                    {n.puntuacion}<span className="text-[9px] font-normal opacity-60">/10</span>
                  </span>
                  <div className="flex gap-1">
                    {n.verificado && <span className="bg-beige text-stone text-[8px] px-1.5 py-0.5 rounded-full">✓</span>}
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-full ${n.abierto ? 'bg-[#E4F0E8] text-forest' : 'bg-beige text-pale'}`}>
              ...
              <div className="flex gap-2 px-4 pb-2 overflow-x-auto scrollbar-hide">
                {(n.precios || []).slice(0, 3).map(p => (
                  <div key={p.id} className="bg-[#F0EDE7] rounded-lg px-2.5 py-1.5 flex-shrink-0">
                    <p className="text-[9px] text-muted">{p.nombre}</p>
                    <p className="text-[11px] font-medium text-carbon">${Number(p.precio).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              <div className="px-4 py-2 border-t border-[#F0EDE7] flex justify-between items-center">
                <div className="flex items-center gap-1">
                  {[1,2,3,4,5].map(i => (
                    <div key={i} className={`w-1.5 h-1.5 rounded-full ${i <= n.rango ? 'bg-rust' : 'bg-beige-dark'}`} />
                  ))}
                  <span className="text-[9px] text-muted ml-1">{n.rango_label}</span>
                </div>
                <span className="text-[9px] text-pale">{n.reportes} reportes</span>
              </div>
            </div>
          </Link>
        ))}
