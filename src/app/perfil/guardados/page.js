...
export default function GuardadosPage() {
  const guardados = negocios.filter(n => n.verificado)
    ...
    <div className="flex-1 bg-cream flex flex-col overflow-hidden">
      ...
      <div className="px-5 pb-4 bg-cream flex-shrink-0 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-serif text-xl font-bold text-carbon">Negocios guardados</h1>
        </div>
      </div>
          ...
          <Link key={n.id} href={`/negocio/${n.id}`}>
            <div className="bg-card rounded-2xl px-4 py-3.5 border border-border flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-muted font-light">{n.municipio} · {n.puntuacion}/10</p>
              </div>
              <span className="text-rust text-xl">♥</span>
            </div>
          </Link>
        ))}
        <p className="text-[11px] text-pale font-light text-center py-4">{guardados.length} negocios guardados</p>
      </div>
      <BottomNav />
    </div>
  )
